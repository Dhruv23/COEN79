...
namespace coen79_lab8
{
    ...
    public:
        ...
        deque_iterator(value_type** initial_block_pointers = nullptr, value_type** initial_block_pointers_end = nullptr,
            ...
            : block_pointers(initial_block_pointers), block_pointers_end(initial_block_pointers_end),
              first_bp(initial_first_bp), last_bp(initial_last_bp),
              front_ptr(initial_front_ptr), back_ptr(initial_back_ptr),
              bp_array_size(initial_bp_array_size), block_size(initial_block_size),
              cursor(initial_cursor), current_boundary(initial_current_boundary), current_block_pointer(initial_current_block_pointer)
        {}
        ...
        {
            deque_iterator temp = *this; // Save the current state of the iterator
            ++(*this); // Use the prefix increment operator to advance the iterator
