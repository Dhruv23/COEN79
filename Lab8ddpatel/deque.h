   ...
   void deque<Item>::operator=(const deque<Item>& source)
    {
        ...
        {
            if (source.block_pointers[i] == NULL)
            {
                block_pointers[i] = NULL;
            }
            else
            {
                block_pointers[i] = new value_type[block_size];
                std::copy(source.block_pointers[i], source.block_pointers[i] + block_size, block_pointers[i]);
            }
        }

        block_pointers_end = block_pointers + bp_array_size - 1;
        first_bp = block_pointers + (source.first_bp - source.block_pointers);
        last_bp = block_pointers + (source.last_bp - source.block_pointers);
        front_ptr = *first_bp + (source.front_ptr - *source.first_bp);  // Corrected line
        back_ptr = *last_bp + (source.back_ptr - *source.last_bp);    // Corrected line
    }
    ...
    void deque<Item>::clear()
    {
        ...
        if (first_bp != NULL) {
            //Iterate over the block pointers between first_bp and last_bp
            for (value_type** bp = first_bp; bp <= last_bp; ++bp) {
                //Delete the data block if it exists
                if (*bp != NULL) {
                    delete[] *bp;
                    *bp = NULL;
                }
...
void deque<Item>::push_front(const value_type& entry)
{
    ...
    if (first_bp == NULL)
    {
        ...
        *first_bp = new value_type[block_size];
        ...
        //front_ptr and back_ptr point to the first entry of the new data block
...
void deque<Item>::push_back(const value_type& entry)
{
    ...
    if (last_bp == NULL)
    {
        ...
        *last_bp = new value_type[block_size];
        ...
        //front_ptr and back_ptr point to the first entry of the new data block
        front_ptr = back_ptr = *last_bp + (block_size / 2);
