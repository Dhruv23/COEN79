...
namespace coen79_lab5
{
    ...
    public:
        ...
        string(const char str[ ] = ""); //done, works
        string(char c);                     // done, works
        string(const string& source); //done, works
        ~string( ); //done
        
        // MODIFICATION MEMBER FUNCTIONS
        void operator +=(const string& addend); //done, works
        void operator +=(const char addend[ ]); //done, works
        void operator +=(char addend); //done, works
        void reserve(size_t n); //done
        string& operator =(const string& source); //done, works
        void insert(const string& source, unsigned int position); //done, works
        void dlt(unsigned int position, unsigned int num); //done, works
        void replace(char c, unsigned int position); //done, works
        void replace(const string& source, unsigned int position); //done, DOESNT WORK
        
        // CONSTANT MEMBER FUNCTIONS
        size_t length( ) const { return current_length; } //done, works
        char operator [ ](size_t position) const; //done, works

        int search(char c) const; //done, works
        int search(const string& substring) const; //done, works
        unsigned int count(char c) const; //done, works
        
        // FRIEND FUNCTIONS
        friend std::ostream& operator <<(std::ostream& outs, const string& source); //done, works
        friend bool operator ==(const string& s1, const string& s2); //done, works
        friend bool operator !=(const string& s1, const string& s2); //done, works
        friend bool operator > (const string& s1, const string& s2); //done, works
        friend bool operator < (const string& s1, const string& s2); //done, works
        friend bool operator >=(const string& s1, const string& s2); //done, works
    ...
    // NON-MEMBER FUNCTIONS for the string class
    string operator +(const string& s1, const string& s2);//done, works
    string operator +(const string& s1, const char addend[ ]); //done, works
    std::istream& operator >> (std::istream& ins, string& target); //done,DOESNT WORK
}
